...
def print_summary(title_text, results):
    widths = (6, 19, 10, 10)
    n_cols = len(widths)
    ...
    title = _c(title_text, _CYAN + _BOLD)
...
def main():
    ap = argparse.ArgumentParser(description="全国运营商测速：选择运营商，一键测速全国所有该运营商节点")
    ap.add_argument("--core", action="store_true",
                    help="Core 模式：不加载沙箱、不做 GitHub 反代判断，仅本机执行核心测速/延迟/丢包")
    ap.add_argument("--mode", default=None,
                    help="测速模式：1=全国移动, 2=全国联通, 3=全国电信（也可直接写运营商名）；不指定则交互选择")
    ...
    operator = None
    if args.mode:
        operator = OPERATOR_MODES.get(args.mode)
        if not operator:
            print("[!] 无效的 --mode 值: %s，可选: 1(移动) 2(联通) 3(电信)" % args.mode)
            sys.exit(1)
    else:
        print("")
        print(_c("===== 选择测速模式 =====", _CYAN + _BOLD))
        print("  1) 全国移动")
        print("  2) 全国联通")
        print("  3) 全国电信")
        idx = choose_index("请输入编号（回车退出）: ", 3)
        if idx is None:
            return
        operator = ["移动", "联通", "电信"][idx]

    cand = pick_nodes(nodes, operator=operator)
    if not cand:
        print("[!] 节点表中没有匹配 %s 的节点" % operator)
        return
    ...
    results = []
    print("")
    _info("全国%s 共 %d 个节点" % (operator, len(cand)))
    for i, node in enumerate(cand, 1):
        print("")
        _divider("[%d/%d] 正在测试: %s (%s:%s)" % (
            i, len(cand), node.get("name", ""), node["ip"], node.get("port")))
        r = run_operator_test(node, args.imei, args.bandwidth, args.seconds, args.threads,
                              progress_enabled, tty, args.timeout)
        results.append(r)
        if not r["ok"]:
            _warn("%s 失败/超时: %s" % (node.get("name", ""), r.get("error", "未知")))
            if r.get("server_log"):
                _warn("  服务器返回日志: %s" % r["server_log"])

    print_summary("全国%s测速汇总" % operator, results)
